...
def _build_class_indices_by_file(Y_merged, groups):
    """
    Build global trial indices for each class, both overall and per subject.

    Label convention inherited from the original script:
        0 = right_hand
        1 = foot

    The indices refer to rows in X, X_eeg, and Y after concatenation.
    """
    Y_merged = np.asarray(Y_merged)
    groups = np.asarray(groups)

    class_indices = {
        "right_hand": np.where(Y_merged == 0)[0],
        "foot": np.where(Y_merged == 1)[0],
    }

    class_indices_by_file = {}
    for dataset_label in sorted(set(groups.tolist())):
        file_mask = groups == dataset_label
        class_indices_by_file[dataset_label] = {
            "right_hand": np.where(file_mask & (Y_merged == 0))[0],
            "foot": np.where(file_mask & (Y_merged == 1))[0],
        }

    return class_indices, class_indices_by_file
...
# When the same file is imported by another script, __name__ takes the value of the module's filename
    ...
    final_dict = {
        ...
        "X_eeg": X_eeg_merged,                           # (N_total, C, T), chopped band-passed epochs
        "class_label_names": {0: "right_hand", 1: "foot"},
        "class_indices": class_indices,                  # global epoch indices by class
        "class_indices_by_file": class_indices_by_file,  # subject-specific epoch indices by class
    ...
    print(f"Merged epoch shape: X_eeg={final_dict['X_eeg'].shape}")
    print("Trial counts by file:", final_dict["trial_counts_by_file"])
    print("Class counts:", {
        name: int(len(indices))
        for name, indices in final_dict["class_indices"].items()
    })

    # Export single merged pickle
    result_root = "/mnt/c/Users/scana/Desktop/gpc_work/data"
