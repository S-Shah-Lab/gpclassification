...
def main() -> None:
    ...
    for nf in nfs:
        for fold_i, fold_dict in enumerate(folds_dicts):
            ...
            w_tag     = "spatialFilter_trainable" if not args.no_w_trainable else "spatialFilter_fixed"
            run_dir   = (
                results_base
                / dataset_label
                / f"shuffle_{bool(args.shuffle_input)}"
                / align_tag
                / f"ard_{bool(args.ard)}"
                / f"kernel_{args.kernel_type.lower()}"
                / w_tag
                / f"nf_{nf}"
                / f"fold_{fold_i}"
            )
            ...
            # Determine W initialisation
            if args.csp:
                spatialFilter_init = _csp_filters_from_covs(X_train, Y_train, nf=nf)
            else:
            ...
            try:
                ...
                failed_jobs += 1
                print(f"[WARN] Failed — nf={nf}, fold={fold_i}: {exc}")
